<?php ob_start(); // Temporisation de ce qui suis ?>
<style>
  h1 {
    text-align: center;
  }
...
<?php
  ...
  include_once("fonction.php"); // Appelle des fonctions php

  $site = $_GET['site']; // Attribution du site
  ...
  // Nombre de panneau pour ce site
  ...
  // Nombre de sentier pour ce site
  ...
  // Nombre d'aménagment de valorisation pour ce site
  ...
  // Nombre de clôture pour ce site
  ...
  // Nombre de barrière pour ce site
  ...
  // Nombre d'aménagement de gestion pour ce site
  ...
  if ($nb_panneau != 0) {
    // Récupération des informations sur les panneaux
  ...
  // Si il y a des sentiers
  if ($nb_sentier != 0) {
    // Récupération des informations sur les sentiers
    ...
    // Récupération de la sommes des longueur des sentier
    ...
    // Récupération de la sommes  des longueur des senties mise en place par le CEN
  ...
  // Si il y a des aménagement de valorisation
  if ($nb_amgtValo != 0) {
    // Récupération des informations sur les aménagement de valorisation
  ...
  // Si il y a des clôtures
  if ($nb_cloture != 0) {
    // Récupération des informations sur les cloture
    ...
    // Récupération de la sommes des longueurs des clotures
  ...
  // Si il y a des barrières
  if ($nb_barriere != 0) {
    // Récupération des informations sur les barrières
  ...
  // Si il y a des aménagments de gestion
  if ($nb_amgtGest != 0) {
    // Récupération des informations sur les aménagements de gestion
...
<!-- Affichage d'un message si il n'y a aucun équipement sur ce site -->
...
<!-- Concernant les panneau -->
<div class="panneau">
<?php if ($nb_panneau != 0): ?>
  <?php $pann_SP = ucfirst(Singulier_Pluriels($nb_panneau, 'panneau')); ?>
  <h3><?=$nb_panneau ?> <?=$pann_SP?></h3> <!-- Affichage du nombre de panneaux -->
    <table>
      <tr>
        <th>Mise en place</th>
        <th>État</th>
    ...
    <?php foreach ($res_info_panneau AS $info_panneau): ?>
      <?php $date_amgt = date('j-m-Y', strtotime($info_panneau['date_amgt'])) ?>
          <tr>
            <td><?=$date_amgt ?></td> <!-- Affichage de la date de mise en place -->
            <td><?=$info_panneau['etat']?></td> <!-- Affichage de l'état -->
            <td><?=$info_panneau['type']?></td> <!-- Affichage du type -->
          </tr>
    <?php endforeach; ?>
    </table>
<?php endif; ?>
...
<?php if ($nb_sentier != 0): ?>
  <?php $sent_SP = ucfirst(Singulier_Pluriels($nb_sentier, 'sentier')); ?>
  <h3><?=$long_sentier->longueur ?> mètre de <?=$sent_SP?></h3> <!-- Affichage de la sommes de la longueur des sentier -->
  <table>
    <tr>
      <th>Mise en place</th>
      <th>État</th>
  ...
  <?php foreach ($res_sentier_info AS $sentier_info): ?>
    ...
    <tr>
      <td><?=$date_amgt ?></td> <!-- date de mise en place -->
      <td><?=$sentier_info['etat']?></td> <!-- etat -->
      <td><?=$sentier_info['gest']?></td> <!-- getionnaire -->
      <td><?=$sentier_info['chem']?></td> <!-- cheminement -->
      <td> <!-- acces pmr -->
      ...
      <td><?=$sentier_info['diff']?></td> <!-- difficulté -->
      <td><?=$sentier_info['longueur']?> m</td> <!-- longueur -->
    </tr>
  <?php endforeach; ?>
  </table>
  <?php if ($sent_cen != 0): ?>
...
<!-- Concernant les aménagements de valorisation -->
<div class="amgtValo">
<?php if ($nb_amgtValo != 0): ?>
  <?php $amgtValo_SP = ucfirst(Singulier_Pluriels($nb_amgtValo, 'aménagement')); ?>
  <h3><?=$nb_amgtValo ?> <?=$amgtValo_SP?></h3> <!-- nombre -->
  <table>
    <tr>
      <th>Mise en place</th>
      <th>État</th>
  ...
  <?php $date_amgt = date('j-m-Y', strtotime($amgt_valo_info['date_amgt'])) ?>
    <tr>
      <td><?=$date_amgt ?></td> <!-- mise en place -->
      <td><?=$amgt_valo_info['etat']?></td> <!-- etat -->
      <td><?=$amgt_valo_info['libe']?></td> <!-- type -->
    </tr>
  <?php endforeach; ?>
  </table>
<?php endif; ?>
...
<!-- Concernant les clotures -->
<div class="cloture">
<?php if ($nb_cloture != 0): ?>
  <?php $clot_SP = ucfirst(Singulier_Pluriels($nb_cloture, 'clôture')); ?>
    <h3><?=$long_cloture_cen->longueur?> mètre de <?=$clot_SP?></h3> <!-- nombre -->
    <table>
      <tr>
        <th>Mise en place</th>
        <th>État</th>
    ...
    <?php $date_amgt = date('j-m-Y', strtotime($cloture_info['date_amgt'])) ?>
      <tr>
        <td><?=$date_amgt ?></td> <!-- mise en place -->
        <td><?=$cloture_info['etat']?></td> <!-- etat -->
        <td><?=$cloture_info['mobi']?></td> <!-- mobilité -->
        <td><?=$cloture_info['fils']?></td> <!-- type de fils -->
        <td><?=$cloture_info['pote']?></td> <!-- type de poteau -->
        <td><?=$cloture_info['longueur']?> m</td> <!-- longueur -->
      </tr>
    <?php endforeach; ?>
    </table>
<?php endif; ?>
...
<?php if ($nb_barriere != 0): ?>
  <?php $barriere_SP = ucfirst(Singulier_Pluriels($nb_barriere, 'barrière')); ?>
  <h3><?=$nb_barriere ?> <?=$barriere_SP?></h3> <!-- nombre -->
  <table>
    <tr>
      <th>Mise en place</th>
      <th>État</th>
  ...
  <?php $date_amgt = date('j-m-Y', strtotime($info_barriere['date_amgt'])) ?>
    <tr>
      <td><?=$date_amgt ?></td> <!-- mise en place -->
      <td><?=$info_barriere['etat']?></td> <!-- etat -->
      <?php $dimension = Singulier_Pluriels($info_barriere['dime'], 'mètre') ?>
      <td><?=$info_barriere['dime']?> <?=$dimension ?></td> <!-- dimension -->
      <td><?=$info_barriere['mobi']?></td> <!-- mobilité -->
      <td><?=$info_barriere['stru']?></td> <!-- structure -->
    </tr>
  <?php endforeach; ?>
  </table>
<?php endif; ?>
...
<!-- Concernant les aménagements de gestion -->
<div class="amgtGest">
<?php if ($nb_amgtGest != 0): ?>
  <?php $amgtGest_SP = ucfirst(Singulier_Pluriels($nb_amgtGest, 'aménagement')); ?>
  <h3><?=$nb_amgtGest ?> <?=$amgtGest_SP?></h3> <!-- nombre -->
  <table>
    <tr>
      <th>Mise en place</th>
      <th>État</th>
  ...
  <?php $date_amgt = date('j-m-Y', strtotime($info_amgt_gest['date_amgt'])) ?>
    <tr>
      <td><?=$date_amgt ?></td> <!-- mise en place -->
      <td><?=$info_amgt_gest['etat']?></td> <!-- etat -->
      <td><?=$info_amgt_gest['libe']?></td> <!-- type -->
    </tr>
  <?php endforeach; ?>
  </table>
<?php endif; ?>
...
<?php
  $content = ob_get_clean(); // Fin de la temporisation, attribution du contenu à une variable et effacement de la mémoire tempon.
